...
export function NewProjectDialog() {
  ...
  const { trigger, isMutating } = useSWRMutation('/api/v1/admin/project',
    (url, { arg }: {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className={cn('sm:max-w-[425px]', {
        'opacity-70 pointer-events-none': isPending || isMutating,
      })}>
        <DialogHeader>
          <DialogTitle>Create a new Project</DialogTitle>
          <DialogDescription>
            Speed up your monorepeo now !
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={(e: React.FormEvent<NameAndSlugForm>) => {
          e.preventDefault()
          const { name, slug } = e.currentTarget.elements
          trigger({ name: name.value, slug: slug.value }, {
            onSuccess: () => {
              start(() => {
                setOpen(false)
                router.refresh()
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-3 items-center gap-4">
              <Label htmlFor="slug" className="flex items-center gap-2 text-gray-600">
                Slug
                <TooltipProvider>
                  <Tooltip>
